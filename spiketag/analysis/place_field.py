...
class place_field(object):
    ...
    def __init__(self, logfile, session_id=0, v_cutoff=5):
    ...
    def get_field(self, spk_time, neuron_id, kernlen=21, std=3):
        ...
        '''
        spk_tw = np.searchsorted(self.ts, spk_time[neuron_id])
        spk_tw = np.vstack((spk_tw-1, spk_tw)).T
        idx = np.delete(spk_tw[:,1], self.v_still_idx)
    ...
    def plot_fields(self, N):
        cluNo = self.fields.keys()[-1]
        fig = plt.figure(figsize=(N*4, (cluNo/N+1)*4))
        for i in range(1, cluNo):
            ax = fig.add_subplot(cluNo/N+1, N, i)
